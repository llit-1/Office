...
const tableStateKey = "tt-factory-list";

const columns: Column<TTListItem>[] = [
  { key: "name", label: "Наименование", responsivePriority: 0 },
  { key: "address", label: "Адрес", defaultVisible: true, responsivePriority: 3 },
  {
    label: "Дата открытия",
...
export default function FactoryLocationsTab() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const result = await callApi(get<TTListItem[]>("/TT/list", { params: { group: "factory" } }));
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) =>
        includesNormalized(
          `${item.name ?? ""} ${item.type ?? ""} ${item.address ?? ""} ${item.rkCode ?? ""}`,
          debouncedSearchText,
        ),
      ),
